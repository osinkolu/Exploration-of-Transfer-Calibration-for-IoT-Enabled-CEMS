//.........................SOURCE CODES FOR EXPOSITORY PROJECT - FINAL DEPLOYMENT..............................................................................
...
// -------------------What you need to change if you want to implement this project-----------------------------------//

// 1. Set your Username and Password
// 2. Set your WEB3.Storage bearer token
// 3. URL for calibration model

// ------------------ If you work for chemotronix, you may want to ignore the above---------------------------------//

// 4. Device name.
// 5. Set your Thingspeak API key
...
#include <WiFiClientSecureBearSSL.h> // Include the BearSSL library Secure client with SSL for https conversations
...
String apiKey = "############## USE Your API KEY #############################"; // Delta - Enter your Write API key from ThingSpeak - Removed secrets
float co2_prediction; //This would help collect response from ML model at AWS Elastic Beanstalk
const char *Device_name = "Alpha"; // replace with the device name
...
const char* url = "https://api.web3.storage/upload"; // Blockchain Host is web3 storage
...
const char* ml_host = "###########################################################################"; Host your OWN API
const int port = 80;
...
const char fingerprint[] PROGMEM = "12 67 35 20 3A 19 14 BC 39 16 8D 3E 1A EA 4B 63 3D 3C 40 4F"; // AS at 31st July 2023 SHA1 for web3.storage
...
// Skip headers and get the response body
      ...
      co2_prediction = responseBody.toFloat();
...
//---------------------------------------------------Web 03 Storage Data Pushing-----------------------------------------------------------------------------------------------------//
  ...
  httpsClient.setInsecure(); // Accept any certificate (only for testing purposes, not recommended for production)


  Serial.println(url); // Confirm the host for web03 storage as declared earlier
...
//------------------------------------------------------------- Current Post Request to web3 storage (JSON) --------------------------------------------------------------------------------------//
 ...
 content["MQ135(RAW)"]=adc3;
 content["MQ135_SENSOR_DIGITAL(DISCRETE)"]=MQ135_sensor_digital;
...
// content["CARBON MONOXIDE AVG CONC(PPM)"]=carbon_monoxide;
 content["CARBON DIOXIDE AVG CONC(PPM)"]=co2_prediction;
...
//-------------------------------------------------------------------------------  Serialize JSON document
 ...
 serializeJson(content, content_json);
    ...
    http.begin(httpsClient, url); // Use client with http.begin() for HTTPS
    http.addHeader("X-NAME", formattedDate + formattedTime +" "+ Device_name);
    http.addHeader("Authorization", "################################################ USE YOUR OWN API KEY ################################################################");
    http.addHeader("Content-Type", "application/json");

    int httpResponseCode = http.POST(content_json);
...
//--------------------------------------------------------------------serial confirmation of Post------------------------------------------------------------------------------------
    ...
    delay(5000); // Send the request every 5 seconds (adjust as needed)
